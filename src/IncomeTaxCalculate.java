public class IncomeTaxCalculate {
    ...
    public IncomeTaxCalculate(double anIncome, double aStatus) {
        income = anIncome;
        status = aStatus;
    }
    public double getTax(){
        ...
        if(status == Constants.SINGLE){
            if(income <= Constants.RATE1_SINGLE_LIMIT){
                tax1 = Constants.RATE1_SINGLE_LIMIT * income;
            }
            else{
                tax1 = Constants.RATE1 * Constants.RATE1_SINGLE_LIMIT;
                tax1 = Constants.RATE2 * (income - Constants.RATE1_SINGLE_LIMIT);
            }
        }
        else{
